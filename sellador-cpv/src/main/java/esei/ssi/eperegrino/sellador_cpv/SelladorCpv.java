...
final class SelladorCpv {
	...
	public static void main(final String[] args) {
		try {
			...
			File bytesClavePrivadaAlbergue;
			...
			bytesClavePrivadaAlbergue = pedirFichero("Escriba la ruta al fichero con la clave privada del albergue", teclado, System.out, true);
			...
			map.put("Motivación", incidencias);
			...
			Actor.ALBERGUE.setClavePrivada(Files.readAllBytes(bytesClavePrivadaAlbergue.toPath()));
			...
			System.err.println("Ha ocurrido un error durante la creación de la credencial");
                ...
                Logger.getLogger(SelladorCpv.class.getName()).log(Level.SEVERE, null, ex);
            }
	}
			...
			throws GeneralSecurityException, InvalidKeySpecException, IOException
	{
		...
		paqueteCpv.anadirBloque(TITULO_BLOQUE_DATOS_SELLO_ALBERGUE, datosEncriptados);
		paqueteCpv.anadirBloque(TITULO_BLOQUE_CLAVE_SELLO_ALBERGUE, claveCifradorEncriptada);
		paqueteCpv.anadirBloque(TITULO_BLOQUE_RESUMEN_SELLO_ALBERGUE_ENCRIPTADOS, resumenEncriptadoDatos);
